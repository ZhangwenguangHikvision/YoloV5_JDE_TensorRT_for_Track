...
void tracker::update(const DETECTIONS &detections, int max_que_class)
{
#ifdef MY_DEBUG
    clock_t start,end;
    start = clock();
    ...
    printf("update time cost: %f ms!\n", (double)(end-start)/CLOCKS_PER_SEC *1000);
    start = clock();
#endif
    for(int& detection_idx:unmatched_detections) 
    {
        this->_initiate_track(detections[detection_idx], max_que_class);
    }

#ifdef MY_DEBUG
    end = clock();
    printf("_initiate_track time cost: %f ms!\n", (double)(end-start)/CLOCKS_PER_SEC *1000);
    start = clock();
#endif
    ...
    for (Track& track:tracks) 
    {
        ...
        FEATURESS t = FEATURESS(0, REID_FEATURE_SIZE);
        int features_size = track.features.rows();
        if(features_size > 0)
        {
            track.current_feature = track.features.row(features_size-1);
        }
        track.features = t;
    }
    ...
    printf("partial_fit time cost: %f ms!\n", (double)(end-start)/CLOCKS_PER_SEC *1000);
#endif
}
...
void tracker::_initiate_track(const DETECTION_ROW &detection, int max_que_class)
{
    ...
    this->tracks.push_back(Track(mean, covariance, this->_next_idx, this->n_init,
                                 ...
                                 detection.confidence, max_que_class, detection.iloop));
    _next_idx += 1;
}
...
{
    FEATURESS features(detection_indices.size(), REID_FEATURE_SIZE);
    int pos = 0;
