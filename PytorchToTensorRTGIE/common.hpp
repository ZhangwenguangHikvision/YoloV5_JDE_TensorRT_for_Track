...
ILayer* JDE(INetworkDefinition *network, std::map<std::string, Weights>& weightMap, ILayer* layer17, ILayer* layer20, ILayer* layer23) 
{
    Weights emptywts{ DataType::kFLOAT, nullptr, 0 };
    auto conv1 = convBlock(network, weightMap, *layer17->getOutput(0), 256, 3, 1, 1, "model.24.proj1.0");

    auto conv2 = convBlock(network, weightMap, *layer20->getOutput(0), 256, 3, 1, 1, "model.24.proj2.0");
    IDeconvolutionLayer* deconv3 = network->addDeconvolutionNd(*conv2->getOutput(0), 256, DimsHW{ 4, 4 }, weightMap["model.24.proj2.1.weight"], emptywts);
    deconv3->setStrideNd(DimsHW{ 2, 2 });
    deconv3->setNbGroups(256);
    deconv3->setPaddingNd(DimsHW{ 1, 1 });

    auto conv4 = convBlock(network, weightMap, *layer23->getOutput(0), 256, 3, 1, 1, "model.24.proj3.0");
    IDeconvolutionLayer* deconv5 = network->addDeconvolutionNd(*conv4->getOutput(0), 256, DimsHW{ 8, 8 }, weightMap["model.24.proj3.1.weight"], emptywts);
    deconv5->setStrideNd(DimsHW{ 4, 4 });
    deconv5->setNbGroups(256);
    deconv5->setPaddingNd(DimsHW{ 2, 2 });

    ITensor* inputTensors1[] = { conv1->getOutput(0), deconv3->getOutput(0), deconv5->getOutput(0)};
    auto cat6 = network->addConcatenation(inputTensors1, 3);

    auto conv7 = convBlock(network, weightMap, *cat6->getOutput(0), 256, 3, 1, 1, "model.24.node.0");
    IConvolutionLayer* conv8 = network->addConvolutionNd(*conv7->getOutput(0), 512, DimsHW{ 1, 1 }, weightMap["model.24.node.1.weight"], weightMap["model.24.node.1.bias"]);
    assert(conv8);
    conv8->setStrideNd(DimsHW{ 1, 1 });

    IShuffleLayer* permute1 = network->addShuffle(*conv8->getOutput(0));
    assert(permute1);
    permute1->setReshapeDimensions( Dims3{ 76, 136, 512 });
    return permute1;
}
