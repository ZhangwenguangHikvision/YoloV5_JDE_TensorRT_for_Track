...
static const int OUTPUT_SIZE2 = 512*76*136;
const char* INPUT_BLOB_NAME = "data";
const char* OUTPUT_BLOB_NAME = "prob";
const char* OUTPUT_BLOB_NAME_2 = "embing";
...
ICudaEngine* createEngine_l(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt) {
    ...
    std::map<std::string, Weights> weightMap = loadWeights("../jde.wts");
    Weights emptywts{ DataType::kFLOAT, nullptr, 0 };
    ...
    // yolo layer 0
    IConvolutionLayer* det0 = network->addConvolutionNd(*bottleneck_csp17->getOutput(0), 3 * (Yolo::CLASS_NUM + 5), DimsHW{ 1, 1 }, weightMap["model.25.m.0.weight"], weightMap["model.25.m.0.bias"]);
    ...
    //yolo layer 1
    IConvolutionLayer* det1 = network->addConvolutionNd(*bottleneck_csp20->getOutput(0), 3 * (Yolo::CLASS_NUM + 5), DimsHW{ 1, 1 }, weightMap["model.25.m.1.weight"], weightMap["model.25.m.1.bias"]);
    ...
    //yolo layer 2
    IConvolutionLayer* det2 = network->addConvolutionNd(*bottleneck_csp23->getOutput(0), 3 * (Yolo::CLASS_NUM + 5), DimsHW{ 1, 1 }, weightMap["model.25.m.2.weight"], weightMap["model.25.m.2.bias"]);

    auto yolo = addYoLoLayer(network, weightMap, det0, det1, det2);
    yolo->getOutput(0)->setName(OUTPUT_BLOB_NAME);
    network->markOutput(*yolo->getOutput(0));
#ifdef USE_EMBING
    auto embing = JDE(network, weightMap, bottleneck_csp17, bottleneck_csp20, bottleneck_csp23);
    embing->getOutput(0)->setName(OUTPUT_BLOB_NAME_2);
    network->markOutput(*embing->getOutput(0));
#endif
    // Build engine
    builder->setMaxBatchSize(maxBatchSize);
    config->setMaxWorkspaceSize(16 * (1 << 20));  // 16MB
    ...
    Int8EntropyCalibrator2* calibrator = new Int8EntropyCalibrator2(1, INPUT_W, INPUT_H, 
        "/mnt/data/MOT17/train/MOT17-02-FRCNN/img1/", "int8calib.table", INPUT_BLOB_NAME);
...
int main(int argc, char** argv) {
...
#ifdef USE_FP16
    engine_name = "yolov5" + engine_name + "_fp16.engine";
#elif defined(USE_INT8)
    engine_name = "yolov5" + engine_name + "_int8.engine";
#endif
    ...
    file_names.clear();
    file_names.push_back("ori_00100.jpg");
    ...
    CHECK(cudaMalloc(&buffers[outputIndex2], BATCH_SIZE * OUTPUT_SIZE2 * sizeof(float)));
#endif
    // Create stream
    cudaStream_t stream;
    ...
    for (int f = 0; f < (int)file_names.size(); f++) {
        ...
        for (int b = 0; b < fcount; b++) {
            ...
            printf("%s\n","_" + file_names[f - fcount + 1 + b]);
            printf("%d  %d  %d  %d  \n", (int)file_names.size(), f, fcount, b);
        }
        fcount = 0;
    }
